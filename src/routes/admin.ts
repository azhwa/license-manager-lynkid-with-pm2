...
adminRoutes.get('/mapping', async (c) => {
  const result = await c.env.DB.prepare('SELECT id, title_pattern, duration_days, plan_type, max_devices, is_active, created_at FROM product_mapping ORDER BY created_at DESC').all();
