...
async function handleWebhook(c: Context<AppContext>, accountSlug?: string) {
  ...
  if (!messageId || !email || !refId || !item?.title || typeof grandTotal !== 'number') return jsonError(c, 400, 'Missing required Lynk.id fields');
  ...
  const valid = await verifyLynkSignature(c.req.header('X-Lynk-Signature') ?? null, grandTotal, refId, messageId, merchantKey);
  const accountId = account?.id ?? null;
  const log = await c.env.DB.prepare('INSERT INTO webhook_logs(lynk_account_id, message_id, raw_payload, signature_valid, processed, error_message) VALUES (?, ?, ?, ?, 0, ?) RETURNING id').bind(accountId, messageId, rawPayload, valid ? 1 : 0, valid ? null : 'Invalid signature').first<{ id: number }>();
  ...
  const transactionMessageId = account ? `${account.id}:${messageId}` : messageId;
  ...
  const mapping = await getMapping(c.env.DB, item.title);
